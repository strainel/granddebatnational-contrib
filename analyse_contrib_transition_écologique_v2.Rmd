...
kable(dt, caption = "Type de contributeur")
```

```{r nbdep, echo=FALSE, fig.height = 8, fig.width = 8}
dt <- data %>% count (dep)
dt$pct <- round(dt$n / sum(dt$n) * 100,2)
plot(dep)
choroLayer(spdf = dep, df = dt, spdfid = "CODE_DEPT", 
           dfid = "dep", var = "n", border = NA, 
           col = carto.pal("wine.pal", 6), legend.pos = "topleft", 
           add = TRUE, method = "geom", nclass = 6, legend.title.txt = "Contribution par département")
...
dt <- data %>% count (Q1F)
dt$pct <- round(dt$n / sum(dt$n) * 100,2)
kable(dt, caption = vars2_value[1])
```
...
dt <- data %>% count (Q3)
dt$pct <- round(dt$n / sum(dt$n) * 100,2)
kable(dt, caption = vars2_value[3])
```
...
dt <- data %>% count (Q5)
dt$pct <- round(dt$n / sum(dt$n) * 100,2)
kable(dt, caption = vars2_value[5])
```
...
dt <- data %>% count (Q9)
dt$pct <- round(dt$n / sum(dt$n) * 100,2)
kable(dt, caption = vars2_value[9])
```
...
dt <- data %>% count (Q11)
dt$pct <- round(dt$n / sum(dt$n) * 100,2)
kable(dt, caption = vars2_value[11])
```
